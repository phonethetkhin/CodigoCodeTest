...
class UpcomingRemoteMediator(
    ...
    override suspend fun load(
        loadType: LoadType, state: PagingState<Int, UpComingMovieEntity>
    ): MediatorResult {
        ...
        try {
            val apiResponse = api.getUpcomingMovies(API_KEY, page)
            apiResponse.body()
            ...
            withContext(Dispatchers.IO) {
                db.withTransaction {
                    ...
                    if (loadType == LoadType.REFRESH) {
                        db.remoteKeyDao().upComingDeleteAll()
                        db.movieDao().deleteAllUpComing()
                    }
                    ...
                    db.remoteKeyDao().upComingInsertAll(keys)
                    db.movieDao().insertAllUpComing(results)
                }
            }
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.id?.let { movieId ->
                db.remoteKeyDao().upComingRemotes(movieId)
            }
        }
    }

    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, UpComingMovieEntity>): RemoteKeyEntity? {
        // Get the first page that was retrieved, that contained items.
        // From that first page, get the first item
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { repo ->
                // Get the remote keys of the first items retrieved
                db.remoteKeyDao().upComingRemotes(repo.id)
            }
    }

    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, UpComingMovieEntity>): RemoteKeyEntity? {
